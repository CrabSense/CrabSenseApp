...
class Boxes extends Table {
  ...
  IntColumn get capacity => integer().named('capacity').withDefault(const Constant(0))();

  /// Species string: blueCrab | mudCrab | softShell
  TextColumn get species => text().named('species').withLength(max: 50)();

  RealColumn get averageWeight => real().named('average_weight').withDefault(const Constant(0))();

  /// Status string: active | inactive | maintenance | harvested
  TextColumn get status =>
      text().named('status').withLength(max: 50).withDefault(const Constant('active'))();

  DateTimeColumn get createdAt => dateTime().named('created_at')();

  DateTimeColumn get lastVideoAt => dateTime().named('last_video_at').nullable()();
...
class Crabs extends Table {
  ...
  RealColumn get weight => real().named('weight').withDefault(const Constant(0))();

  /// Molting status: preMolt | molting | postMolt | hardShell
  TextColumn get moltingStatus => text().named('molting_status').withLength(max: 50)();

  /// Health status: normal | disease | stress | unknown
  TextColumn get healthStatus =>
      text().named('health_status').withLength(max: 50).withDefault(const Constant('unknown'))();
...
class WaterQualityReadings extends Table {
  ...
  RealColumn get temperature => real().named('temperature').withDefault(const Constant(0))();
  ...
  RealColumn get salinity => real().named('salinity').withDefault(const Constant(0))();

  DateTimeColumn get timestamp => dateTime().named('timestamp')();

  BoolColumn get isAlertTriggered =>
      boolean().named('is_alert_triggered').withDefault(const Constant(false))();
...
class Alerts extends Table {
  ...
  TextColumn get sourceType => text().named('source_type').withLength(max: 50).nullable()();
  ...
  TextColumn get status =>
      text().named('status').withLength(max: 20).withDefault(const Constant('unread'))();
...
class OperationLogs extends Table {
  ...
  TextColumn get boxIds => text().named('box_ids').withDefault(const Constant('[]'))();
  ...
  TextColumn get photoUrls => text().named('photo_urls').withDefault(const Constant('[]'))();

  DateTimeColumn get timestamp => dateTime().named('timestamp')();

  TextColumn get operatorId => text().named('operator_id')();

  TextColumn get operatorName => text().named('operator_name').withLength(max: 255)();
...
class Harvests extends Table {
  ...
  /// Total weight in kg.
  RealColumn get totalWeight => real().named('total_weight').withDefault(const Constant(0))();

  IntColumn get crabCount => integer().named('crab_count').withDefault(const Constant(0))();

  /// Quality grade: gradeA | gradeB | gradeC
  TextColumn get qualityGrade => text().named('quality_grade').withLength(max: 20)();
...
class Sales extends Table {
  ...
  TextColumn get transactionId => text().named('transaction_id').withLength(max: 100)();

  TextColumn get buyerName => text().named('buyer_name').withLength(max: 255)();

  TextColumn get buyerContact => text().named('buyer_contact').withLength(max: 255).nullable()();

  /// Quantity in kg.
  RealColumn get quantity => real().named('quantity').withDefault(const Constant(0))();
  ...
  TextColumn get paymentMethod => text().named('payment_method').withLength(max: 50)();

  /// Payment status: pending | completed | cancelled
  TextColumn get paymentStatus =>
      text().named('payment_status').withLength(max: 50).withDefault(const Constant('pending'))();
...
class SyncQueue extends Table {
  ...
  TextColumn get operationType => text().named('operation_type').withLength(max: 100)();

  /// ID of the entity this operation targets.
  TextColumn get entityId => text().named('entity_id')();

  /// Entity type: box | crab | alert | operationLog | harvest | sale | inspection
  TextColumn get entityType => text().named('entity_type').withLength(max: 50)();
  ...
  TextColumn get status =>
      text().named('status').withLength(max: 20).withDefault(const Constant('pending'))();
  ...
  IntColumn get priority => integer().named('priority').withDefault(const Constant(1))();
...
class Inspections extends Table {
  ...
  TextColumn get relatedVideoId => text().named('related_video_id').nullable()();
  ...
  TextColumn get syncStatus =>
      text().named('sync_status').withLength(max: 20).withDefault(const Constant('pending'))();
...
class Videos extends Table {
  ...
  TextColumn get status =>
      text().named('status').withLength(max: 50).withDefault(const Constant('pending'))();
...
class AiDetections extends Table {
  ...
  TextColumn get feedbackStatus => text().named('feedback_status').withLength(max: 20).nullable()();
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (m, from, to) => DatabaseMigrationManager.onUpgrade(m, from, to, this),
    beforeOpen: (db) async {
